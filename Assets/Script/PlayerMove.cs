...
public class PlayerMove : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetButtonDown("Jump") && !anim.GetBool("IsJumping"))  //! animator.GetBool("isJumping") -  매개변수가 false인 경우
                                                                            //무한점프를 방지
        {
        ...
        if (Input.GetButton("Horizontal"))//player1의 Horizontal 입력을 받았을때
        {
            sr.flipX = Input.GetAxisRaw("Horizontal") == -1;//좌우반전(FlipX)의 값을 바꿈
        ...
        if (Input.GetKeyDown(KeyCode.Space) && canSmashing)
        {
